# coding=utf-8
__all__ = ["show_k_lines", "get_name_by_code", "get_daily_by_code"]
...
def get_daily_by_code(stock_code='000001.SZ', start_date=None, end_date=None):
    if len(stock_code) < 1:
        print(f"!!!get_daily_by_code: wrong par: stock_code={stock_code}")
        return
    if start_date is None:
        start_date = '20050101'
    if end_date is None:
        end_date = str(datetime.datetime.now().date().strftime("%Y%m%d"))
    if not (len(start_date) == 8 and len(end_date) == 8):
        print(f"!!!get_daily_by_code: wrong par: start_date={start_date} end_date={end_date}")
    ...
    try:
        ...
        if len(stock_code) < 6:
            print(f'!!!! get_daily_by_code error: stock_code is invalid: {stock_code}')
            return df_daily
        else:
            stock_code = stock_code[0:6]

        sql_text = text(
            f'''select * from daily where ts_code like \'{stock_code}___\' 
            and trade_date between \'{start_date}\' and \'{end_date}\' ''')
...
class InterCandle:
    def __init__(self, data, my_style):
        ...
        self.fig = mpf.figure(style=my_style, figsize=(12, 6), facecolor=(0.82, 0.83, 0.85))
        fig = self.fig
        self.ax1 = fig.add_axes([0.08, 0.15, 0.88, 0.70])
        self.ax2 = fig.add_axes([0.08, 0.05, 0.88, 0.10], sharex=self.ax1)
    ...
    def refresh_plot(self, idx_start, idx_range):
        ...
        buy_signal = plot_data[['buy_price']]
        ap.append(mpf.make_addplot(buy_signal, type='scatter', markersize=100, marker='v', color=(0.75, 0.6, 0.6),
                                   ax=self.ax1))
    ...
    def refresh_texts(self, display_data):
        ...
        self.t4.set_text(f'{np.round(display_data["_change"], 3)}')
        ...
        elif display_data['_change'] < 0:  # 如果今日变动额小于0，即今天价格低于昨天，今天价格显示为绿色
